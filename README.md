...
- Servicio base para Firestore y contacto por WhatsApp.
- Reglas iniciales en `firestore.rules`.
- Firebase Storage y fotografías están fuera de esta primera versión.
...
5. Publica las reglas con Firebase CLI: `firebase deploy --only firestore:rules`.
6. No necesitas activar Storage ni cambiar al plan Blaze para esta versión.
7. Para otorgar administración, asigna el custom claim `admin: true` desde un backend confiable o Firebase Admin SDK. Nunca desde el formulario público.
